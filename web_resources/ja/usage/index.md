...
最終更新: 2026/07/02 Egaroucid 7.8.2 / 7.8.3
...
何手目、手番、現在の石数、<a href="#表示_情報エリアへの表示 > 定石">定石名</a>、<a href="#設定_レベル">AIのレベル</a>、<a href="#表示_情報エリアへの表示 > 最善筋">AIが想定している読み筋</a>、対局開始からの経過時間、を表示しています。

AIが黒にも白にも着手しない設定のときは、手番表示に分析モードであることが表示されます。
...
対局に関する情報(プレイヤ名、メモ)を閲覧できます。この画面では編集できないので、編集したい場合は編集ボタンを押してください。
...
設定では、AIがどれくらい石損する手を選んでよいかを調整できます。バージョン7.8.1からは、局面の進み具合に応じて石損のさせ方を細かく調整できるようになりました。例えば序盤は小さな石損で変化させ、中盤はほぼ完璧に打ち、終盤は大きめの石損もたまにする、といった設定ができます。

設定内容はプロファイルとして保存できます。練習用、弱めの対局用、ランダム性を大きくした対局用など、目的ごとに設定を切り替えられます。
...
強制する進行は「AIの進行を強制 > 進行設定」から自由に設定できます。「追加」ボタンを押し、好きな進行を入力し、「登録」ボタンを押すことで登録できます。なお、棋譜が間違っている場合、登録ボタンがグレーアウトして押せなくなります。
...
各進行は任意の名前をつけたフォルダに分けられます。フォルダ内にさらにフォルダを作ることもできますし、フォルダや進行を親フォルダに移動させることも可能です。
...
登録してある進行の左上のバツボタンを押すと、その進行の登録を削除できます。右のチェックボックスによって、その進行を消さずとも選ばないようにできます。また、右側の鉛筆ボタンで編集も可能です。

それぞれの進行には「重み」を設定できます。あるフォルダ内に複数の進行を登録した場合、どの進行を選ぶかは確率的に決まりますが、重みが大きければ大きいほど、その進行が選ばれる確率が高くなります。具体的には、ある盤面が登録した進行に現れ、かつ登録されている手が複数あるとき、それらの手の重みの集合を$\{W\}$、$m$という手の重みを$w$とすると、

$mが選ばれる確率 = \frac{w}{\sum{\{W\}}}$

となります。なお、フォルダにつけた重みは、そのフォルダ内の手を選ぶ確率を表します。

つまり、以下の画像の例だと並行取りを選ぶ確率を1とすると、縦取りは5の確率で選ばれます。斜め取りはチェックボックス側でオフにしているので選ばれません。縦取りか並行取りかを確率的に決定したら、そのフォルダ内で同様の処理を繰り返し、最終的に選ぶ手を決定します。

<div class="centering_box">
    <img class="pic2" src="img/force_ai_opening_setting5.png">
</div>

強制された進行をなぞっている場合、情報エリアに「強制中」と表示されます。バージョン7.8.1からは、登録した進行をなぞっている状態、ユーザが進行から外れた状態、登録した進行が最後まで終わった状態がよりわかりやすく表示されるようになりました。

なお、<a href="#設定_AIに石損させる">AIに石損させる</a>設定とAIの進行強制が同時にオンの場合は、AIの進行強制が優先されます。
...
各種設定をプロファイルとして保存し、あとから切り替えられます。対局用、解析用、配信用など、目的ごとに設定を分けたいときに使えます。

プロファイルを切り替えると、そのプロファイルに保存された設定が読み込まれます。頻繁に切り替える設定がある場合は、プロファイルを分けておくと便利です。
...
<ul>
    ...
    <li>操作
        <ul>
            ...
            <li><a href="#操作_進む/戻る">最初の局面に戻る</a> Home</li>
            <li><a href="#操作_進む/戻る">最後の局面に進む</a> End</li>
            <li><a href="#操作_ランダム生成局面に戻る">ランダム生成局面に戻る</a> Shift+Home</li>
...
ヒントの数値につける配色を2つ用意しています。どちらの配色でも最善手を太字で表示しています。
...
レベルとBook精度の表示をオフにし、さらに[うみがめ数の表示](#表示_合法手への表示 > うみがめ数)をオフにすると、盤面中央に大きく評価値が表示される「シンプルモード」になります。このモードでは、値が完全読みになった場合のみマスの右上にチェックマークがつきます。
...
バージョン7.8.2からは、うみがめ数の計算条件として累積石損とスコア範囲を設定できます。累積石損を0にすると、各局面で局所的な最善手だけを辿る一般的なうみがめ数になります。累積石損までの石損で、かつ盤面のスコア範囲が設定内にある進行の数を数え、拡張したうみがめ数を表示できます。設定は変更後に自動で反映されます。
...
直前の手で返した石を表示できます。どの石が返ったのかを見返したいときや、解説用に盤面を表示するときに便利です。
...
マウスホバーした手に打ったときに返る石をわかりやすく表示します。
...
着手の順番を1手目から1、2、3と表示します。盤面と棋譜をあわせて表示したい場合に有用です。着手後の盤面を表示した「盤面形式」と、着手した手番を表示した「棋譜形式」の2通りの表示方法があります。
...
対局開始後の黒番・白番それぞれの使った時間が表示できます。

バージョン7.8.2からは、タイマーの開始、一時停止、リセットを手動で操作できます。人間の手番では開始ボタンを押すまでタイマーは動きません。AIの手番でまだ一度もタイマーを開始していない場合は、自動で開始されます。

「表示 > 情報エリアへの表示 > タイマー (分)」から時間制限を設定できます。左端の「なし」にすると時間制限なしになり、1分から60分まで1分単位で選択できます。時間制限に達したプレイヤの時間表示は点滅します。
...
バージョン7.8.2からは、ランダム局面生成やXOT局面生成の途中に作られる局面について、通常の対局グラフに値を表示しないようにしました。生成された局面そのものからの対局や分析に集中しやすくなります。なお、「ランダム局面のグラフを表示」チェックボックスをオンにすると、XOT等のランダム局面を生成する途中のグラフも表示されます。
...
終盤20手での各プレイヤーの石損を表示します。終盤として40手目から60手目(デフォルト)、41手目から60手目を選択できます。
...
なお、ショートカットキーとして、単に右矢印キーを押すかDキーを押すだけでも局面を進められます。また、左矢印キーかAキーを押すと局面を戻せます。さらに、Homeキーを押すと最初の局面に戻れ、Endキーで最後の盤面に進みます。
...
<a href="#操作_ランダム局面を生成">ランダム局面を生成</a>機能でランダム局面を生成してそのまま対局を継続した場合、ランダム生成直後の局面に戻ることができます。ショートカットキーはデフォルトでShift+Homeです。
...
生成ボタンを押すと自動で中盤局面が生成されます。生成手数を変更することで、何手目の局面を生成するかを選択できます。また、生成する局面のスコア範囲を設定できるようにしています。下の画像では、18手打った後の局面で黒にとって-38から-21まで、つまり白にとって+21から+38の範囲の局面を生成する設定になっています。ただし、中盤探索で大体スコア範囲に収まるように盤面を生成しているため、厳密に生成盤面がスコア範囲に収まるかは保証できません。バージョン7.8.1からは、スコア範囲の幅を最小2石まで狭められるようになりました。

バージョン7.8.2からは、ランダム局面を作る途中のグラフはデフォルトでは表示されないようになっています。「表示>グラフエリアへの表示>グラフ>ランダム局面のグラフを表示」をオンにすると、ランダム局面生成中のグラフも表示されます。

ランダム局面生成はデフォルトのショートカットキーとしてCtrl+Rを割り当てています。
...
この機能では、各着手に対して68%の確率で6石損以下、95%の確率で12石損以下の手を選ぶようにしており、突拍子のない局面は出にくくなっています。

具体的な手の選び方は以下の通りです。

まず、指定された手数-2までの各着手に対して以下の計算によって手を選んでいます。

平均0、標準偏差6の正規分布に従ってランダムに数値を出力する関数を$\mathrm{randdst_{0,6}()}$とします。
...
なお、ここでは強い手を選ぶ必要がないため、レベル1の探索を使用しています。book登録値がある場合はbook値を使い、ない場合はレベル1相当の2手読みの評価値を使用しています。

バージョン7.8.2からは、生成した局面が指定したスコア範囲に入っているかを、設定したAIレベルを使って最後に確認するようになりました。指定された手数までの最後の2手は、指定されたスコア範囲に収まるような手があればその手を、なければスコア範囲に近い手を選ぶようにしています。このスコア範囲への調整には<a href="#設定_レベル">設定 > レベル</a>で設定したAIレベルを使用します。最後から2手目はレベル15、最後の1手は設定したAIレベルで探索します。ただし、設定したAIレベルが15未満の場合、最後から2手目も設定したAIレベルまで下げて探索します。また、通常部分のレベル1も設定したAIレベルを超えないようにしています。



### XOT局面を生成

XOTの開始局面をランダムに生成できます。メニューの「操作 > XOT局面を生成」から実行でき、ショートカットキー、ショートカットボタン、マウス追加ボタンにも割り当てられます。

XOTはMatthias Berg氏が作成・公開している開始局面集で、Egaroucidには作者の許可を得て同梱しています。通常の初期局面ではなく、XOT局面から練習対局や解析を始めたいときに使います。
...
AIの計算が終わらない場合など、「計算停止」ボタンを押すと計算を一時停止できます。停止中は情報エリアに「計算停止中」と表示されます。停止前に計算できていた結果はそのまま表示されます。

「計算再開」ボタンを押すと計算を再開します。停止中に中断されたヒント、最善筋、AIの注目する場所、うみがめ数、Book精度、対局分析、AI着手計算などは、再開後に必要に応じて再計算されます。
...
保存した対局を一覧で管理できます。ドキュメントフォルダ内のEgaroucid/gamesフォルダを、Egaroucid内のライブラリとして閲覧します。

フォルダの作成、名前変更、移動、コピー、切り取り、貼り付け、削除ができます。複数選択やドラッグ&ドロップにも対応しているので、保存した対局が増えてきたらフォルダ分けして整理できます。

バージョン7.8.2では対局ライブラリの操作がエクスプローラ風に整理されました。空白部分のクリックで選択解除、Ctrl+Aで全選択、Ctrl+C/Ctrl+X/Ctrl+Vでコピー・切り取り・貼り付けができます。また、ごみ箱を有効にすると、削除した対局をいったんごみ箱へ移動し、ごみ箱内で完全に削除できます。
...
オセロクエストの公開対局を、ユーザー名から検索して取り込めます。ユーザー名を入力し、対局モードを5分、1分、XOTから選択して検索します。

検索結果では、対局日時、対局者、結果などを確認できます。対局を選択して取り込むと、Egaroucidの盤面にその対局が反映されます。選択した対局は「対局ライブラリに保存」から保存先フォルダを選んで保存できます。最後に検索したユーザー名と対局モードは記憶されます。
...
保存日時、対局者、結果、メモがまとまって表示されるので、ダブルクリックすると読み込めます。また、左上のバツ印を押すと対局を削除できます。

対局数が多い場合にはスクロールできます。マウスホイールのほか、上下ボタンやPage Up、Page Downでもスクロールできます。ドラッグ&ドロップによるフォルダ分け・並び替えが可能です。さらに、鉛筆ボタンからその場で対局ファイルやフォルダ名の編集が可能です。空のフォルダは左上にバツボタンが現れ、削除できますが、何かが入っているファイルは消せません。その場合、中のものをドラッグ&ドロップで移動させるか消すかすると消せるようになります。

プレイヤ名やメモが長い場合、適宜省略されます。また、勝ったプレイヤの名前は赤、負けたプレイヤの名前は青で表示されます。引き分けは橙です。終局まで保存されていない対局はスコアが??-??と表示され、名前の背景色がつきません。

バージョン7.8.1からは、直前に開いていたフォルダを記憶します。対局読み込み画面を開き直したときに、前回見ていたフォルダから探せます。
...
バージョン7.8.1からは、盤面画像に評価値を追加できたり、座標の表示有無の選択、盤面の外枠の色の選択ができるようになりました。
...
対局をEgaroucid独自形式で保存します。最終結果やグラフの情報、対局開始からの経過時間も自動で保存されます。

黒番と白番の名前とメモを書き、「本筋を保存」ボタンで保存できます。なお、「設定局面までを保存」では、<a href="#画面の見方_盤面">盤面エリア</a>に表示されている局面までの情報を保存できます。つまり、局面を戻してからこのボタンで対局を保存すると、戻したところまでが保存されます。

Ctrl+Vによってコピペが可能です。ショートカットキーとして、Tabキーで入力エリア切り替え、Escで「戻る」操作ができます。バージョン6.5.2からカーソル移動に対応しました。メモの入力は10000文字までの制限があります。

バージョン7.7.0からは任意にフォルダを作れるようになりました。保存先フォルダを選択して、「ここに保存」ボタンを押してください。フォルダはダブルクリックで移動できます。また、フォルダや対局情報をドラッグ&ドロップで任意のフォルダに移動できます。

バージョン7.8.2からは、保存先の選択や保存後の整理を<a href="#入出力_対局ライブラリ">対局ライブラリ</a>と同じ仕組みで扱えるようになりました。オセロクエストから取り込んだ対局も、同じライブラリへ保存できます。

対局を読み込みたい場合は<a href="#入出力_入力 > 対局読み込み">対局読み込み</a>から一覧を確認し、読み込んでください。

ドキュメントフォルダ内のEgaroucid/gamesフォルダにデータが保存されています。
...
### Book操作 > 現在の棋譜をBookに追加

現在の局面まで至る棋譜を指定深さまでBookに追加します。その際、各局面で最善手も追加で登録します。EdaxにおけるBook Storeに近い機能です。
...
バージョン7.8.2からは「Book修正(Edax準拠)」が復活しました。通常のBook修正とは別に、Edax形式のleafも候補として扱いながらBook値を修正します。Edax形式Bookとのやり取りを重視する場合に使ってください。ただし、完全な互換性は担保しません。
...
バージョン7.8.0で、Edax形式のBookを読み込むときの処理を一部変更しました。
...
バージョン7.8.2からは、Book書き出し中に「強制停止」ボタンが表示されます。途中で停止した場合でも、既存の出力先Bookファイルを壊さないように一時ファイル経由で書き出します。Book書き出し中にウィンドウを閉じた場合も、書き出し停止を要求してから終了します。

Edax形式で書き出す場合は、「追加計算あり」と「追加計算なし」を選択できます。「追加計算あり」は、Edax形式として成立するように必要なleafを追加計算してから書き出します。「追加計算なし」は、Egaroucid内部Bookに既に存在する局面情報だけを使い、探索や追加計算を行わずにEdax形式へ変換します。

また、Edax形式で追加計算ありを選んだ場合は、リーフレベルを指定できます。リーフレベルは、Edax形式として必要になったleafを再計算するときに使うAIレベルです。
...
バージョン7.8.0以降、Edax形式のBookを書き出すときの処理を一部変更しています。
...
Egaroucidの設定や保存済みデータを別のPCへ移すための機能です。「データの書き出し」で、指定したフォルダに移行用フォルダを作成します。作成されるフォルダ名はEgaroucid_Settings_日時の形式です。

「データの読み込み」では、書き出したEgaroucid_Settings_...フォルダを指定して読み込みます。設定は上書きされ、ドキュメントフォルダ内の対局やBookなどのデータはコピーされます。Bookなど一部のデータは、読み込み後にEgaroucidを再起動すると反映されます。
